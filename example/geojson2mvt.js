...
for(var i=0; i<=parseInt(myArgs[5]); i++){
	//Generate First Tile Set
	if(i < parseInt(myArgs[5])){
		json = JSON.parse(fs.readFileSync(myArgs[0]+'('+i+').geojson', "utf8"));
		var options = {
		  layers: {
			layer0: json,
		  },
		  rootDir: "Tiles/"+myArgs[0].substring(start,end)+"(1)",
	...
	//Generate Second Tile Set
	var j = i+1;
	if(j > parseInt(myArgs[5])) j = i;
	json2 = JSON.parse(fs.readFileSync(myArgs[0]+'('+j+').geojson', "utf8"));
	var options2 = {
	  layers: {
			layer0: json2,
		},
		rootDir: "Tiles/"+myArgs[0].substring(start,end)+"(2)",
		bbox : [parseFloat(myArgs[1]),parseFloat(myArgs[2]),parseFloat(myArgs[3]),parseFloat(myArgs[4])],	//[south,west,north,east]
		zoom : {
			min :i,
			max :i
	...
	console.time('geojson2mvt'+j.toString());
	// build the static tile pyramid
	geojson2mvt(options2);
	console.timeEnd('geojson2mvt'+j.toString());
}
